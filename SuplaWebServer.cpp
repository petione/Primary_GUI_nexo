...
void SuplaWebServer::begin() {
  ...
  if (configManager.ConfigMode == normal) {
  ...
  // String www_username1 = String(configManager.getLogin());
  // String www_password1 = String(configManager.getLoginPassword());
...
void SuplaWebServer::set() {
  ...
  for (int i = 0; i < httpServer.arg("max_ds18b20").toInt(); i++) {
    ...
    if (i >= configManager.get(KEY_MAX_DS18B20)->getValueInt()) {
      configManager.deleteKey(ds_key.c_str());
      configManager.deleteKey(ds_name_key.c_str());
  ...
  /* configManager.setWIFI_SSID(httpServer.arg("wifi_ssid").c_str());
    configManager.setWifiPassword(httpServer.arg("wifi_pass").c_str());
    configManager.setLogin(httpServer.arg("modul_login").c_str());
    configManager.setLoginPassword(httpServer.arg("modul_pass").c_str());
    configManager.setHostName(httpServer.arg("supla_hostname").c_str());
    configManager.setSuplaServer(httpServer.arg("supla_server").c_str());
    configManager.setSuplaEmail(httpServer.arg("supla_email").c_str());

    for (int i = 0; i < MAX_DS18B20; i++) {
     String ds = "ds18b20_channel_id_";
     ds += i;
     configManager.setDeviceAddress(httpServer.arg(ds).c_str(), i);

     String ds_name = "ds18b20_name_id_";
     ds_name += i;
     configManager.setDeviceName(httpServer.arg(ds_name).c_str(), i);
    }

    configManager.setMaxDs18b20(httpServer.arg("max_ds18b20").toInt());
...
String SuplaWebServer::supla_webpage_start(int save) {
  ...
  content += "<i><label>MAX DS18b20</label><input name='max_ds18b20' type='number' placeholder='0' step='1' min='1' max='20' value='" + String(configManager.get(KEY_MAX_DS18B20)->getValue()) + "'></i>";
  content += "</div>";
...
String SuplaWebServer::supla_webpage_search(int save) {
  ...
  content += "<h3>Temperatura</h3>";
  for (int i = 0; i < configManager.get(KEY_MAX_DS18B20)->getValueInt(); i++) {
    double temp = 0;//get_temperature(ds18b20_channel[i].channel, 0);
    String ds_key = KEY_DS;
    ds_key += i;
